...
| Release notes | `release-notes/v7.[0-7].md` |
| Migration guides | `troubleshooting/v7-migration-guide.md`, `troubleshooting/v7.3-migration-guide.md`, `troubleshooting/v7.7-migration-guide.md` |
...
rules/src/                    # Server-side game logic
  ...
  ├── DekalRules.ts           # Main rules class
  ├── DekalSetup.ts           # Initial game setup
  └── DekalOptions.ts         # Game configuration
...
yarn build    # type check + build the app
yarn lint     # eslint on rules and app
yarn test     # vitest
```
...
Only this game's own keys belong here. The framework strings live in the `common` and `credits`
namespaces, served by the Game Park website. If you need one of them from the game code, pass the
namespace explicitly: `t('Close', { ns: 'common' })`.
...
Add `?locale=fr` (or `es`, `en`...) to the URL to test the texts in a given language.
